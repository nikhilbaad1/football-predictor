...
Lexical (BM25) retrieval only — no embeddings yet, deliberately. The dense half
gets added once there is a number saying what this half achieves on its own;
see ADR 0014 and scripts/eval_retrieval.py.
...
def main() -> int:
    ...
    hits = search(args.query, team=args.team, season=args.season, limit=args.limit)
    if not hits:
    ...
    print(f'"{args.query}"  ({scope})\n')

    for hit in hits:
        heading = f" › {hit['heading']}" if hit["heading"] else ""
        print(f"  [{hit['rank']}] {hit['score']:6.2f}  {hit['title']}{heading}")
